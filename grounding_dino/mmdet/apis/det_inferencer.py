...
class DetInferencer(BaseInferencer):
    ...
    def __call__(
        ...
        ori_inputs = self._inputs_to_list(glob.glob(inputs+"/*"))
        ...
        for ori_imgs, data in (track(inputs, description='Inference')
            ...
            # save predictions into labels:
            data = {}
            all_per_img_bboxes = []

            image = cv2.imread(ori_imgs[0])
            for each_original_bbox in all_boxes[0]:
                part_normalized_bbox = [each_original_bbox[1] / image.shape[0],
                                        each_original_bbox[0] / image.shape[1],
                                        (each_original_bbox[3] - each_original_bbox[1]) / image.shape[0],
                                        (each_original_bbox[2] - each_original_bbox[0]) / image.shape[1]]
                all_per_img_bboxes.append(part_normalized_bbox)

                start_point = (int(each_original_bbox[0]), int(each_original_bbox[1]))  # Top left corner
                end_point = (int(each_original_bbox[2]), int(each_original_bbox[3]))  # Bottom right corner
            ...
            cv2.imwrite("{0}/{1}.png".format(out_dir, os.path.basename(ori_imgs[0])[:-4]), image)

            data['part_normalized_bbox'] = all_per_img_bboxes
            np.save('{0}/{1}.npy'.format(out_dir,  os.path.basename(ori_imgs[0])[:-4]), data)
    ...
    def compute_TPFP(self, img_name, img_info, predition_bbox):
        # gt_folder = os.path.dirname(os.path.dirname(single_input))

        with open('/home/zoeyc/github/mmdetection/data/cabinets/annotations/test.json', 'r') as f:
            gt_data = json.load(f)
        image_id = None
        ...
        for img in gt_data['images']:
            if img['file_name'] == img_name:
                image_id = img['id']
                break
        # Extract ground truth boxes for the specific image
        if image_id is not None:
            gt_boxes = [anno['bbox'] for anno in gt_data['annotations'] if anno['image_id'] == image_id]
        else:
            gt_boxes = []
        ...
        for pred_box in predition_bbox:
            is_tp = False
            for gt_box_ori in gt_boxes:
                gt_box = [gt_box_ori[0], gt_box_ori[1], gt_box_ori[0]+gt_box_ori[2], gt_box_ori[1]+gt_box_ori[3]]
